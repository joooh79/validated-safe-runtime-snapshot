...
export function getUnsupportedActionError(action: WriteAction): AirtableAdapterError {
  // Case actions
  if (action.entityType === 'case') {
    return canonConfirmRequiredError(
      `action_${action.actionType}`,
      'case',
      'This Case action remains outside the minimal active Stage 5 subset and is still blocked pending fuller Case identity, transition, and sequencing semantics',
    );
  }
  ...
  if (action.entityType === 'link') {
    return canonConfirmRequiredError(
      `action_${action.actionType}`,
      'linking',
      'This explicit link action remains outside the minimal active Stage 6 subset and stays blocked pending broader authoritative write-side, payload shape, and replay/idempotence decisions',
    );
  }
  ...
  if (action.entityType === 'snapshot') {
    ...
    if (
      ...
      return canonConfirmRequiredError(
        `snapshot_${branch}`,
        branch,
        `${branch} branch remains blocked: schema exists, but writable payload mapping, option coverage, record-name behavior, and identity/update-vs-create semantics are not yet activated`,
      );
    }
  }
  ...
  return unsupportedActionError(
    action.actionType,
    'Action type not yet supported by Airtable adapter',
  );
}
