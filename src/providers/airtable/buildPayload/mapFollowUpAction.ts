...
import type { AirtableCreateRequest } from '../types.js';
...
export interface MapFollowUpActionInput {
  action: WriteAction;
...
export type MapFollowUpActionOutput =
  | { success: true; request: AirtableCreateRequest }
  | { success: false; error: AirtableAdapterError };

export function mapFollowUpAction(
  input: MapFollowUpActionInput,
): MapFollowUpActionOutput {
  const {
    action,
    registry,
    resolvedRefs,
  ...
  const patientLinkValue = buildLinkedRecordCell({
    ...
    missingRefMessage:
      'create_post_delivery_follow_up requires a resolved patient record reference at execution time',
  });
  ...
  const visitLinkValue = buildLinkedRecordCell({
    ...
    table: 'Post-delivery Follow-ups',
    missingRefMessage:
      'create_post_delivery_follow_up requires a resolved visit record reference at execution time',
  });
  ...
  const caseLinkValue = buildLinkedRecordCell({
    ...
    table: 'Post-delivery Follow-ups',
    missingRefMessage:
      'create_post_delivery_follow_up requires a resolved case record reference at execution time',
  });
  ...
  const followUpResult = maybeNormalizeSelect(
    intended?.followUpResult,
