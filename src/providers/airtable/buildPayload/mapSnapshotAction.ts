...
export function mapSnapshotAction(input: MapSnapshotActionInput): MapSnapshotActionOutput {
  ...
  switch (action.actionType) {
    case 'create_snapshot': {
      ...
      let tableName: AirtableCreateRequest['table'];
      if (branch === 'PRE') {
        ...
        if (!isSafePlanCreate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_PLAN',
              'PLAN',
              'PLAN create is active only for single-tooth, Case-aware, later-date create flows',
            ),
          };
        }
        ...
        if (!isSafeDoctorReasoningCreate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DR',
              'DR',
              'DR create is active only for single-tooth, Case-aware, later-date create flows',
            ),
          };
        }
        ...
        if (!isSafeDiagnosisCreate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DX',
              'DX',
              'DX create is active only for single-tooth, Case-aware, later-date create flows',
            ),
          };
        }
        tableName = 'Diagnosis';
        ...
        if (!isSafeRadiographicFindingsCreate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_RAD',
              'RAD',
              'RAD create is active only for single-tooth, Case-aware, later-date create flows',
            ),
          };
        }
        ...
        if (!isSafeOperativeFindingsCreate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_OP',
              'OP',
              'OP create is active only for single-tooth, Case-aware, later-date create flows',
            ),
          };
        }
        ...
        return {
          success: false,
          error: canonConfirmRequiredError(
            `snapshot_table_${branch}`,
            branch,
            `Non-PRE snapshot branch ${branch} is still blocked: schema exists, but branch-specific payload mapping, option coverage, record-name behavior, and create-vs-update/idempotence rules are not yet activated`,
          ),
        };
      }
            ...
            : branch === 'DR'
              ...
              : branch === 'DX'
                ...
                : branch === 'RAD'
                  ...
                  : registry.preOpFields.recordName.fieldName
        ] = recordName;
      }
            ...
            : branch === 'DR'
              ...
              : branch === 'DX'
                ...
                : branch === 'RAD'
                  ...
                  : registry.preOpFields.toothNumber.fieldName
        ] = toothNumber;
      }
      ...
      if (branch === 'PRE') {
        ...
        if ('remainingCuspThicknessDecision' in intended) {
          ...
          if (typeof value === 'string') {
            const allowed = Object.values(
              registry.doctorReasoningRemainingCuspThicknessDecisionOptions,
            );
            const normalized = normalizeSelectOption(
              value,
              allowed,
            ...
            if (typeof normalized === 'string') {
              fields[
                registry.doctorReasoningFields.remainingCuspThicknessDecision.fieldName
              ] = normalized;
            }
          }
        }
        ...
        if ('functionalCuspInvolvement' in intended) {
          ...
          if (typeof value === 'string') {
            const allowed = Object.values(
              registry.doctorReasoningFunctionalCuspInvolvementOptions,
            );
            const normalized = normalizeSelectOption(
              value,
              allowed,
        ...
        if ('crackProgressionRisk' in intended) {
          ...
          if (typeof value === 'string') {
            const allowed = Object.values(
              registry.doctorReasoningCrackProgressionRiskOptions,
            );
            const normalized = normalizeSelectOption(
              value,
              allowed,
        ...
        if ('radiographicCariesDepth' in intended) {
          ...
          if (typeof value === 'string') {
            ...
            if (typeof normalized === 'string') {
              fields[
                registry.radiographicFindingsFields.radiographicCariesDepth.fieldName
              ] = normalized;
            }
          }
        }
        ...
        if ('radiographicFractureSign' in intended) {
          ...
          if (typeof value === 'string') {
            ...
            if (typeof normalized === 'string') {
              fields[
                registry.radiographicFindingsFields.radiographicFractureSign.fieldName
              ] = normalized;
            }
          }
        }
        ...
        if ('remainingCuspThicknessMm' in intended) {
          ...
          if (typeof value === 'number') {
            fields[
              registry.operativeFindingsFields.remainingCuspThicknessMm.fieldName
            ] = value;
          }
        }
        ...
        if ('deepMarginalElevation' in intended) {
          ...
          if (typeof value === 'string') {
            ...
            if (typeof normalized === 'string') {
              fields[
                registry.operativeFindingsFields.deepMarginalElevation.fieldName
              ] = normalized;
            }
          }
        }
        ...
        if ('resinCoreBuildUpType' in intended) {
          ...
          if (typeof value === 'string') {
            ...
            if (typeof normalized === 'string') {
              fields[
                registry.operativeFindingsFields.resinCoreBuildUpType.fieldName
              ] = normalized;
            }
          }
        }
    ...
    case 'update_snapshot': {
        ...
        if (!hasExplicitExistingSnapshotTarget(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_PLAN',
              'PLAN',
              'PLAN update remains blocked because no explicit existing PLAN row target was resolved for same-date correction',
            ),
          };
        }
        ...
        if (!isSafePlanUpdate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_PLAN',
              'PLAN',
              'PLAN update is active only for single-tooth, same-date correction flows with an explicit existing PLAN row target',
            ),
          };
        }
        ...
        if (!hasExplicitExistingSnapshotTarget(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DR',
              'DR',
              'DR update remains blocked because no explicit existing DR row target was resolved for same-date correction',
            ),
          };
        }
        ...
        if (!isSafeDoctorReasoningUpdate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DR',
              'DR',
              'DR update is active only for single-tooth, same-date correction flows with an explicit existing DR row target',
            ),
          };
        }
        ...
        if (!hasExplicitExistingSnapshotTarget(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DX',
              'DX',
              'DX update remains blocked because no explicit existing DX row target was resolved for same-date correction',
            ),
          };
        }
        ...
        if (!isSafeDiagnosisUpdate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_DX',
              'DX',
              'DX update is active only for single-tooth, same-date correction flows with an explicit existing DX row target',
            ),
          };
        }
        ...
        if (!hasExplicitExistingSnapshotTarget(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_RAD',
              'RAD',
              'RAD update remains blocked because no explicit existing RAD row target was resolved for same-date correction',
            ),
          };
        }
        ...
        if (!isSafeRadiographicFindingsUpdate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_RAD',
              'RAD',
              'RAD update is active only for single-tooth, same-date correction flows with an explicit existing RAD row target',
            ),
          };
        }
        ...
        if (!hasExplicitExistingSnapshotTarget(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_OP',
              'OP',
              'OP update remains blocked because no explicit existing OP row target was resolved for same-date correction',
            ),
          };
        }
        ...
        if (!isSafeOperativeFindingsUpdate(action)) {
          return {
            ...
            error: canonConfirmRequiredError(
              'snapshot_table_OP',
              'OP',
              'OP update is active only for single-tooth, same-date correction flows with an explicit existing OP row target',
            ),
          };
        }
      ...
      let tableName: AirtableUpdateRequest['table'];
      if (branch === 'PRE') {
        tableName = 'Pre-op Clinical Findings';
      } else {
        return {
...
function isSafePlanCreate(action: WriteAction): boolean {
  return (
...
function isSafePlanUpdate(action: WriteAction): boolean {
  return (
    action.target.branch === 'PLAN' &&
    action.target.toothNumber !== undefined &&
    action.target.toothNumber !== 'all' &&
...
function isSafeDoctorReasoningCreate(action: WriteAction): boolean {
  return (
...
function isSafeDoctorReasoningUpdate(action: WriteAction): boolean {
  return (
    action.target.branch === 'DR' &&
    action.target.toothNumber !== undefined &&
    action.target.toothNumber !== 'all' &&
...
function isSafeDiagnosisCreate(action: WriteAction): boolean {
  return (
...
function isSafeDiagnosisUpdate(action: WriteAction): boolean {
  return (
    action.target.branch === 'DX' &&
    action.target.toothNumber !== undefined &&
    action.target.toothNumber !== 'all' &&
...
function isSafeRadiographicFindingsCreate(action: WriteAction): boolean {
  return (
...
function isSafeRadiographicFindingsUpdate(action: WriteAction): boolean {
  return (
    action.target.branch === 'RAD' &&
    action.target.toothNumber !== undefined &&
    action.target.toothNumber !== 'all' &&
...
function isSafeOperativeFindingsCreate(action: WriteAction): boolean {
  return (
...
function isSafeOperativeFindingsUpdate(action: WriteAction): boolean {
  return (
    action.target.branch === 'OP' &&
    action.target.toothNumber !== undefined &&
    action.target.toothNumber !== 'all' &&
