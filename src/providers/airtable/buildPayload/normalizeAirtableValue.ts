 ...
 */
export function normalizeString(value: unknown): string | AirtableAdapterError {
 ...
 */
export function normalizeDate(value: unknown): string | AirtableAdapterError {
  if (typeof value !== 'string') {
 ...
 */
export function normalizeNumber(value: unknown): number | AirtableAdapterError {
 ...
 */
export function normalizeBoolean(value: unknown): boolean | AirtableAdapterError {
...
export function normalizeSelectOption(
  ...
  if (!allowedOptions.includes(trimmed)) {
    return invalidMappedValueError(
      fieldName,
      value,
      `one of: ${allowedOptions.join(', ')}`,
    );
  }
  return trimmed;
}
 ...
 */
export function normalizeLinkedRef(value: unknown): string | AirtableAdapterError {
  if (typeof value !== 'string') {
 ...
 */
export function isError(value: unknown): value is AirtableAdapterError {
  return (
    typeof value === 'object' &&
    value !== null &&
    'type' in value &&
    (value as { type: string }).type === 'invalid_mapped_value' ||
    (value as { type: string }).type === 'canon_confirm_required' ||
    (value as { type: string }).type === 'missing_option_mapping'
  );
}
