...
import type { AirtableUpdateRequest } from '../types.js';
...
export type MapLinkActionOutput =
  | { success: true; request: AirtableUpdateRequest }
  | { success: false; error: AirtableAdapterError };

export function mapLinkAction(input: MapLinkActionInput): MapLinkActionOutput {
  const { action } = input;
    ...
    case 'link_snapshot_to_visit':
      return {
        ...
        error: canonConfirmRequiredError(
          ...
          'linking',
          `${action.actionType} remains outside the minimal active Stage 7E subset`,
        ),
      };
...
function mapVisitToCaseLink(input: MapLinkActionInput): MapLinkActionOutput {
  const { action, registry, resolvedRefs, requireRuntimeRefs } = input;
  const visitDependencyActionId = action.dependsOnActionIds[0];
  const visitRecordId = getDependentRef(
    visitDependencyActionId,
    resolvedRefs,
    requireRuntimeRefs,
    action.actionType,
    'visit',
  ...
  const caseRecordId = getCaseRef(
    action,
    resolvedRefs,
    requireRuntimeRefs,
    caseDependencyActionId,
  );
...
function mapSnapshotToCaseLink(input: MapLinkActionInput): MapLinkActionOutput {
  const { action, registry, resolvedRefs, requireRuntimeRefs } = input;
  ...
  if (
    ...
    return {
      success: false,
      error: canonConfirmRequiredError(
        'snapshot_case_link_branch',
        action.target.branch || 'unknown',
        'only PRE, PLAN, DR, DX, RAD, and OP snapshot-to-case explicit linking are active in Stage 7E',
      ),
    };
  }

  const snapshotDependencyActionId =
    action.dependsOnActionIds[action.dependsOnActionIds.length - 1];
  const snapshotRecordId = getDependentRef(
    snapshotDependencyActionId,
    resolvedRefs,
    requireRuntimeRefs,
    action.actionType,
    'snapshot',
  );
  const caseDependencyActionId =
    action.dependsOnActionIds.length > 1 ? action.dependsOnActionIds[0] : undefined;
  const caseRecordId = getCaseRef(
    action,
    resolvedRefs,
    requireRuntimeRefs,
...
function getDependentRef(
  ...
  if (!resolvedRef) {
    return canonConfirmRequiredError(
      `link_dependency_${entity}`,
      'linking',
      `${actionType} requires a resolved ${entity} record reference at execution time`,
    );
  }
...
function getCaseRef(
  ...
  if (action.target.caseId && action.target.caseId !== 'NEW') {
    ...
    if (isAdapterError(normalized)) {
      return canonConfirmRequiredError(
        ...
        'Cases',
        'existing case link requires a resolved case record reference',
      );
    }
    return normalized;
  }
  ...
  if (!dependencyActionId) {
    return canonConfirmRequiredError(
      'relationship_source_case_identity',
      'Cases',
      `${action.actionType} requires either an existing case ref or a create_case dependency`,
    );
  }
  ...
  if (!resolvedRef) {
    return canonConfirmRequiredError(
      'relationship_source_case_identity',
      'Cases',
      `${action.actionType} requires the case action to produce a resolved case record reference`,
    );
  }
