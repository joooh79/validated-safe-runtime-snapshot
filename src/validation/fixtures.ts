...
import type { CurrentStateLookupBundle, SnapshotLookupResult } from '../resolution/index.js';
import type { ContinuityIntent, SnapshotBranch, WorkflowIntent } from '../types/core.js';
import type {
  PatientClues,
  VisitContext,
  ToothFindingsItem,
} from '../types/contract.js';
import type { SnapshotBranchIntent } from '../write-plan/rules/buildSnapshotActions.js';
import { createEmptyLookupBundle } from '../resolution/index.js';
import type { GoldenCaseInput } from './types.js';

/**
 * Create a lookup bundle for a specific scenario
 */
export function createScenarioLookupBundle(overrides: Partial<CurrentStateLookupBundle> = {}): CurrentStateLookupBundle {
 ...
 */
export function bundleWithExistingPatient(patientId: string): CurrentStateLookupBundle {
 ...
 */
export function bundleWithSameDateVisit(
  patientId: string,
  visitId: string,
  date: string,
 ...
 */
export function bundleWithNoPatient(): CurrentStateLookupBundle {
  const bundle = createEmptyLookupBundle();
 ...
 */
export function bundleWithDuplicateSuspicion(): CurrentStateLookupBundle {
  const bundle = createEmptyLookupBundle();
 ...
 */
export function bundleWithExistingCase(
  patientId: string,
  caseId: string,
  toothNumber?: string,
): CurrentStateLookupBundle {
...
export function createContractFromScenario(input: GoldenCaseInput): NormalizedContract {
  const workflowIntent = input.contractInputSummary.workflowIntent as WorkflowIntent;
  ...
  const patientClues: PatientClues = {
    existingPatientClaim:
  ...
  const toothItems: ToothFindingsItem[] = (input.contractInputSummary.findings ?? []).map(
    (finding) => ({
      toothNumber: finding.tooth,
      branches: [
        {
          branch: finding.branch as SnapshotBranch,
          payload: {},
        },
      ],
    }),
  ...
  return {
    ...
    workflowIntent,
    continuityIntent:
      (input.contractInputSummary.continuityIntent as ContinuityIntent | undefined) ??
      'none',
    patientClues,
    visitContext,
    findingsContext: {
...
export function createLookupBundleFromScenario(input: GoldenCaseInput): CurrentStateLookupBundle {
  const patientSearch = input.currentStateLookup.patientSearch;
  const visitSearch = input.currentStateLookup.visitSearch;
  ...
  const patientLookup = patientSearch
    ? (() => {
        const lookup: CurrentStateLookupBundle['patientLookup'] = {
  ...
  const sameDateVisitLookup = visitSearch
    ? (() => {
        const lookup: CurrentStateLookupBundle['sameDateVisitLookup'] = {
  ...
  const bundle: CurrentStateLookupBundle = {
    patientLookup,
    sameDateVisitLookup,
    caseLookups:
      caseSearch?.found && caseSearch.caseId && firstFindingTooth
        ? {
            [firstFindingTooth]: {
              found: true,
  ...
  if (snapshotSearch && snapshotSearch.length > 0) {
    bundle.snapshotLookups = snapshotSearch.reduce<
      NonNullable<CurrentStateLookupBundle['snapshotLookups']>
    >((acc, snapshot) => {
      const branch = snapshot.branch as SnapshotBranch;
      acc[branch] ||= {};

      const snapshotLookup: SnapshotLookupResult = {
...
export function createSnapshotBranchIntentsFromScenario(
  ...
  const uniqueFindings = findings.filter(
    (finding, index) =>
      findings.findIndex(
        (candidate) =>
          candidate.branch === finding.branch && candidate.tooth === finding.tooth,
      ) === index,
  );
