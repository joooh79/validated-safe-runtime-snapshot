...
server.listen(port, HOST, () => {
  console.log(
    `[server] listening on http://${HOST}:${port} (defaultProviderMode=${defaultProviderConfig.mode})`,
  );
});
...
function buildOrchestrationRequest(
  pathname: '/preview' | '/execute',
  payload: unknown,
): ApiOrchestrationRequest {
  if (!isRecord(payload)) {
...
function resolveDefaultProviderConfig(): ApiProviderConfig {
