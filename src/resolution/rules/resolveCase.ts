...
export function resolveCase(
  ...
  const caseContext = (toothNumber?: string) => ({
  ...
  if (visitResolution.status === 'create_new_visit') {
    ...
    if (continuityIntent === 'create_case') {
      ...
      return finalizeCaseResolution(
        'create_case',
        [createCaseTarget('create_case', firstTooth, visitDate, reasons)],
        reasons,
      );
    }
    ...
    if (
      ...
      const targets = touchedTeeth.map((toothNumber) =>
        createCaseTarget('create_case', toothNumber, visitDate, reasons),
      );
...
function createCaseTarget(
  status: CaseResolutionTarget['status'],
  toothNumber: string,
  visitDate: string | undefined,
  reasons: string[],
  overrides: Partial<CaseResolutionTarget> = {},
): CaseResolutionTarget {
  return {
    status,
    toothNumber,
...
function finalizeCaseResolution(
