...
import type { SameDateVisitLookupResult } from '../types.js';
...
export function resolveVisit(
  ...
  sameDateLookup: SameDateVisitLookupResult,
): VisitResolution {
  ...
  if (intent === 'new_patient_new_visit' || intent === 'existing_patient_new_visit') {
    if (sameDateLookup.found && sameDateLookup.visitId) {
      ...
      if (correctionConfirmed === null || correctionConfirmed === undefined) {
        ...
        reasons.push(
          'new_visit_intent + same_date_visit_exists + correction_not_confirmed',
        );
        return {
      ...
      if (correctionConfirmed === false && context.visitDate !== context.targetVisitDate) {
        ...
        reasons.push(
          'new_visit_intent + same_date_visit_exists + explicit_keep_new_visit_stance',
        );
        return {
      ...
      if (correctionConfirmed === true) {
        ...
        reasons.push(
          'new_visit_intent + same_date_visit_exists + correction_confirmed_true',
        );
        return {
          status: 'update_existing_visit_same_date',
          resolvedVisitId: sameDateLookup.visitId,
...
function isPatientOnlyUpdate(
  ...
  const hasPatientUpdateFields =
    (contract.patientClues.birthYear !== undefined &&
