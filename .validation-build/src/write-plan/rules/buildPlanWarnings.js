 ...
 */
export function buildPlanWarnings(resolution, actions) {
    ...
    if (resolution.readiness === 'blocked_hard_stop') {
        warnings.push('🛑 Hard stop: write plan execution is blocked due to unresolvable conflict.');
    }
    // Unresolved ambiguity
    if (resolution.ambiguity.hasAmbiguity) {
        warnings.push(`⚠️ Ambiguity detected: ${resolution.ambiguity.ambiguityTypes.join(', ')}`);
    }
    ...
    if (patientAction?.safety.highRiskIdentityAction) {
        warnings.push('ℹ️ Patient action is high-risk identity operation; verify patient details.');
    }
