 ...
 */
export function resolveCorrection(patient, visit) {
    const reasons = [];
    ...
    if (visit.status === 'unresolved_visit_ambiguity') {
        ...
        if (visit.reasons?.some((r) => r.includes('target'))) {
            reasons.push(reason);
            return {
                correctionNeeded: true,
 ...
 */
export function resolveAmbiguity(patient, visit, caseResolution, correctionNeeded) {
