 ...
 */
export function isContractInputValid(input) {
    return (input &&
        typeof input === 'object' &&
 ...
 */
export function isNormalizedContractValid(contract) {
    const errors = [];
    const warnings = [];
    // Required fields
 ...
 */
export function hasValidFindings(contract) {
    return (contract.findingsContext &&
        contract.findingsContext.toothItems &&
        contract.findingsContext.toothItems.length > 0 &&
        contract.findingsContext.toothItems.some((item) => item.branches && item.branches.length > 0));
}
/**
 * Check if patient clues support existing patient resolution
 */
export function supportsExistingPatientResolution(contract) {
 ...
 */
export function supportsNewPatientCreation(contract) {
    const clues = contract.patientClues;
 ...
 */
export function indicatesSameDateVisit(contract) {
 ...
 */
export function indicatesCorrectionActive(contract) {
    const ctx = contract.visitContext;
 ...
 */
export function ensureNoInventedAirtableFields(payload) {
 ...
 */
export function ensureNoInventedAirtableOptions(fieldName, optionValue, canonOptions) {
 ...
 */
export function markCanonConfirmRequired(location, field) {
    return `canon-confirm-required:${location}:${field}`;
}
/**
 * Check if a value is a canon-confirm-required marker
 */
export function isCanonConfirmRequired(value) {
