...
import { buildErrorResponse, buildTerminalResponse, } from './steps/buildTerminalResponse.js';
export async function orchestrateRequest(request) {
    ...
    try {
        ...
        if (plan.readiness !== 'execution_ready') {
            const terminalStatus = plan.readiness === 'preview_only' ? 'no_op' : 'blocked_before_write';
            return buildTerminalResponse({
                request: prepared,
                resolution,
                plan,
        ...
        return buildErrorResponse(fallbackRequestId, error instanceof Error ? error.message : String(error), error);
    }
}
function mapExecutionToTerminalStatus(executionStatus) {
