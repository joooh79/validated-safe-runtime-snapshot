 ...
 */
export async function resolveState(contract, currentStateLookups) {
    const warnings = [
        ...contract.warnings, // Preserve any parsing warnings
    ];
    // Step 1: Resolve patient
    const patientResolution = resolvePatient(contract, currentStateLookups.patientLookup);
    // Step 2: Resolve visit
    const visitResolution = resolveVisit(contract, currentStateLookups.sameDateVisitLookup);
    // Step 3: Resolve case
    const caseResolution = resolveCase(contract, visitResolution, currentStateLookups);
    ...
    const summary = generateResolutionSummary(patientResolution, visitResolution, caseResolution, readiness, interactionMode);
