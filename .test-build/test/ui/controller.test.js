...
import { applyNumericChoice, buildExecuteRequest, deriveExecutionGate, sanitizeWorkingRequest, } from '../../src/ui/controller.js';
...
test('same-date correction choice patches the payload and forces preview again', () => {
    ...
    assert.equal(outcome.nextWorkingRequest?.interactionInput?.correction?.doctorConfirmedCorrection, true);
    assert.equal(outcome.nextWorkingRequest?.interactionInput?.confirmation?.confirmed, undefined);
});
