...
test('update_patient falls back to scanning Patients records when formula lookup finds no direct match', async () => {
    ...
    try {
        ...
        assert.equal(requests.length, 1);
        assert.equal(requests[0]?.type, 'update');
        assert.equal(requests[0] && 'recordId' in requests[0] ? requests[0].recordId : undefined, 'rec_patient_196872');
...
test('create_visit accepts exact Visit type values continue case and follow up', async () => {
    const visitTypes = ['continue case', 'follow up'];
...
test('default mapping registry recognizes the new Cases and Post-delivery Follow-ups schema fields', () => {
    ...
    assert.equal(registry.caseFields.finalProsthesisPlanDate.fieldName, 'Final prosthesis plan date');
    assert.equal(registry.caseFields.latestPostDeliveryFollowUpResult.fieldName, 'Latest post-delivery follow-up result');
    assert.equal(registry.postDeliveryFollowUpFields.followUpResult.fieldName, 'Follow-up result');
